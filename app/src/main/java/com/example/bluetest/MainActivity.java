...
//                i.putExtra(SOCKET, new Gson().toJson(btSocket));
//                //i.putExtra("btsocket", new Gson().toJson(btSocket));
//                //extras.putString(SOCKET, btSocket);
//               // extras.putParcelable(SOCKET, btSocket);
//                //extras.putAll();
//                //extras.put
//                //i.putExtras(extras);
//                startActivity(i);
    ...
    class ConnectBT extends AsyncTask<Void, Void, Void>  // UI thread
    {
        ...
        protected void onPostExecute(Void result) //after the doInBackground, it checks if everything went fine
        {
            ...
            if (!ConnectSuccess) {
                ...
                //Bundle extras = new Bundle();
                //i.putExtra(EXTRA_ADDRESS, address);
               // String personJsonString = new Gson().toJson(btSocket);
                Gson gson = new Gson();
                Type type = new TypeToken<BluetoothSocket>() {}.getType();
                try {
                    String json = gson.toJson(btSocket, type);
                    i.putExtra(IMUActivity.SOCKET, json);
                } catch (Exception e){
                    msg("error");
                }

                //String personJsonString = new Gson().toString(btSocket);
                //i.putExtra(SOCKET, personJsonString);
                //i.putExtra(SOCKET, new Gson().toJson(btSocket));
                msg("Connected.");
                //i.putExtra("btsocket", new Gson().toJson(btSocket));
                //extras.putString(SOCKET, btSocket);
